...
def play(env, policy, time=2000, preprocess=None, nrand=5):
    ...
    for _ in range(nrand):
        frame1, reward1, is_done, _, info = env.step(np.random.choice([RIGHT,LEFT]))
        frame2, reward2, is_done, _, info = env.step(0)
    ...
    for _ in range(time):
        ...
        frame1, _, is_done, _, info = env.step(action)
        frame2, _, is_done, _, info = env.step(0)
